...
contract StableSwap {
    ...
    */
    ...
    function addLiquidity(
        ...
        // Calculatye new liquidity d1
        uint d1 = _getD (new_xs);
        require(d1 > d0, " liquidity didn't increase");

        // Reccalculate D accounting for fee on imbalance
        uint d2;
        if (_totalSupply > 0){
            for (uint i; i < N; ++i){
        ...
        // Shares to mint = (d2 -d0) / d0 * totalSupply
        // d1 >= d2 >= d0
        if(_totalSupply > 0){
    ...
    */ 
    ...
    function _calcWithdrawOneToken(
        ...
        // Recalculate y with xp includeing imbalance fees
